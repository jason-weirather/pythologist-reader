...
class CellFrameInForm(CellFrameGeneric):
    ...
    def scored_calls(self):
        ...
        d = self.get_data('thresholds').reset_index(). \
            merge(self.get_data('cell_measurements').reset_index(),
                  on=['statistic_index', 'feature_index', 'channel_index'])
    ...
    def _read_data(self,
        ...
        _cell_regions = _seg[['Cell ID', 'Tissue Category']].copy().rename(
            columns={'Cell ID': 'cell_index', 'Tissue Category': 'region_label'})
    ...
    def _parse_measurements(self, _seg, channel_abbreviations):
        ...
        keepers2 = [x for x in _seg.columns if
                    re.search('Entire Cell.*\s+\S+ \(Normalized Counts, Total Weighting\)$', x)]
        keepers3 = [x for x in _seg.columns if
                    re.search('\s+\S+ \(Normalized Counts, Total Weighting\)$', x) and x not in keepers2]
        _intensity1 = []
        for cname in keepers2:
            m = re.match('Entire Cell\s+(.*) (Mean|Min|Max|Std Dev|Total) \(Normalized Counts, Total Weighting\)$',
                         cname)
            stain = m.group(1)
        ...
        _intensity2 = pd.DataFrame(_intensity2,
                                   columns=['cell_index', 'channel_label', 'feature_label', 'statistic_label', 'value'])
        ...
        if channel_abbreviations:
            _measurement_channels['channel_abbreviation'] = \
                _measurement_channels.apply(
                    lambda x: x['channel_label'] if x['channel_label'] not in channel_abbreviations else
                    channel_abbreviations[x['channel_label']], 1)
        ...
        _cell_measurements = _intensity.merge(
            self.get_data('measurement_channels')[['channel_label', 'channel_abbreviation']].reset_index(),
            on='channel_label', how='left'). \
    ...
    def _read_images(self, binary_seg_image_file=None, component_image_file=None, verbose=False, require=True,
        ...
        if binary_seg_image_file is not None:
            ...
            if 'ProcessRegionImage' in m.index:
                ...
                self._images[self.processed_image_id] = np.array(
                    pd.DataFrame(img).applymap(lambda x: 0 if x == 255 else 1)).astype(np.int8)
            ...
            if 'Nucleus' in segmentation_images.index and \
                ...
                if color_count < 10:
                    if verbose: sys.stderr.write(
                        "Only found " + str(color_count) + " colors on the membrane looks like legacy\n")
        ...
        if _region.shape[0] == 0:
            ...
            df = pd.DataFrame(
                pd.Series({'region_index': 0, 'image_id': region_id, 'region_size': img.sum()})).T.set_index(
                'region_index')
            temp = self.get_data('regions').drop(columns=['image_id', 'region_size']).merge(df, left_index=True,
                                                                                            right_index=True,
                                                                                            how='right')
    ...
    def _read_component_image(self, filename):
        ...
        temp = self.get_data('measurement_channels').drop(columns=['image_id']).reset_index().merge(df,
                                                                                                    on='channel_label',
                                                                                                    how='left')
    ...
    def _process_tissue_class_map(self, image_description, img):
        ...
        # if value of key 'Entry' is list,
        if image_description['Entry'] == list:
        ...
        for region in regions:
            ...
            self._images[image_id] = np.array(
                pd.DataFrame(img.astype(int)).applymap(lambda x: 1 if x == region else 0)).astype(np.int8)
...
def preliminary_threshold_read(score_data_file, measurement_statistics, measurement_features, measurement_channels,
                               regions):
    ...
    def _parse_binary(_score_data, measurement_statistics, measurement_features, measurement_channels, regions):
        ...
        if 'Stain Component' in _score_data.columns:
            ...
            _score_data = _score_data[
                ['Tissue Category', 'Cell Compartment', 'Stain Component', 'Positivity Threshold']]. \
            ...
            table1 = _score_data[
                ['Tissue Category', 'First Cell Compartment', 'First Stain Component', first_name + ' Threshold']]. \
                rename(columns={
            ...
            table2 = _score_data[
                ['Tissue Category', 'Second Cell Compartment', 'Second Stain Component', second_name + ' Threshold']]. \
                rename(columns={
                'Tissue Category': 'region_label',
    ...
    def _parse_ordinal(_score_data, measurement_statistics, measurement_features, measurement_channels, regions):
        ...
        threshold_columns = [(x, mre.match(x).group(1) + '/' + mre.match(x).group(2)) for x in list(_score_data.columns)
                             if mre.match(x)]
        ...
        _thresholds['gate_label'] = _thresholds.apply(lambda x:
                                                      x['channel_abbreviation'] + ' ' + threshold_dict[
                                                          x['_temp_ordinal']]
                                                      , 1)
    ...
    if is_ordinal:
        _thresholds = _parse_ordinal(_score_data, measurement_statistics, measurement_features, measurement_channels,
                                     regions)
    else:
        _thresholds = _parse_binary(_score_data, measurement_statistics, measurement_features, measurement_channels,
                                    regions)
