...
import pandas as pd
import numpy as np
from mibidata import segmentation as mibi_segmentation, tiff as mibi_tiff

class CellProjectMIBI(CellProjectGeneric):
    def __init__(self,h5path,mode='r'):
        super().__init__(h5path,mode)
        return

    def create_cell_sample_class(self):
        return CellSampleMIBI()
...
class CellSampleMIBI(CellSampleGeneric):
    def __init__(self):
        super().__init__()

    def create_cell_frame_class(self):
        return CellFrameMIBI()
...
class CellFrameMIBI(CellFrameGeneric):
    ...
    def default_raw(self):
        return self.get_raw(feature_label='Total',statistic_label='Mean')

    def read_json(self,input_data_json,run_parameters_json,verbose=False):
    	# take a dictinary format input (compatible with json)
    	self.read_raw(
    		input_data_json['frame_name'],
    		input_data_json['mibi_component_tif_path'],
    ...
    def _read_data(self,
        ...
        _cells = _seg.loc[:,['x_centroid','y_centroid']].copy().reset_index().\
            rename(columns={
            	'label':'cell_index',
            	'x_centroid':'x',
            	'y_centroid':'y'
            	}).applymap(int).set_index('cell_index')
        ...
        _measurement_statistics = pd.DataFrame([[0,'Total'],
        	                                    [1,'Mean']],columns=['statistic_index','statistic_label']).\
                                     set_index('statistic_index')
        self.set_data('measurement_statistics',_measurement_statistics)

        _measurement_features = pd.DataFrame({'feature_label':['Total','Circular Sectors 4','Circular Sectors 8']})
        _measurement_features.index.name = 'feature_index'
        self.set_data('measurement_features',_measurement_features)


        _cell_measurements = _segs.merge(self.get_data('measurement_channels')[['channel_label','channel_abbreviation']].reset_index(),on='channel_label',how='left').\
        ...
        _cell_measurements['cell_index'] = _cell_measurements['cell_index']
